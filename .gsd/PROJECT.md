...
A career command center for AI safety talent. Individuals maintain living profiles and get matched to opportunities with acceptance probability estimates, personalized recommendations, and LLM-generated career actions — self-directed things they can do right now to advance their AI safety career. Local orgs get a self-maintaining CRM as a byproduct. The core insight: people keep profiles updated when they get real value back, not because you ask them to.
...
**Shipped:** v2.0 Course Program Platform (2026-03-11)

v2.0 added a complete course/program platform for BAISH's Technical AI Safety course. Five feature areas: unified prompt system (exercises, activities, polls with three visibility modes), AI learning sidebar (Socratic partner with module context via @convex-dev/agent), facilitator agent (AI copilot with 10 tools and propose-and-approve workflow), live session runner (phases, timers, pairing, real-time presence), and module enhancements (essential/optional materials, audio upload, time-to-session, continue-here markers). 8 new Convex tables, 32 components, 15 backend files in convex/course/.

## Completed Milestone: v2.0 Course Program Platform

**Completed:** 2026-03-11. All 5 slices delivered, all 39 requirements validated.

**Delivered features:**

- ✅ Unified prompt system (one primitive for exercises, session activities, polls, feedback)
- ✅ AI sidebar for participants (Socratic learning partner with full module context)
- ✅ Facilitator agent (AI copilot with tool access, propose-and-approve workflow)
- ✅ Session runner (live phases, timers, write-then-reveal, pairing)
- ✅ Module enhancements (essential/optional, audio, time-to-session, continue-here)

**No active milestone.**

**Tech stack:** Convex + TanStack Start + React 19 + shadcn/ui + Claude Sonnet 4.5/Haiku 4.5 + @convex-dev/agent

**Codebase:** ~130,000 lines TypeScript, ~550 files
...
- LLM-generated personalized career actions (3-5 per person, 8 types) — v1.6
- Action generation alongside matching + on-demand refresh — v1.6
- Action interactions: save, dismiss, in-progress, done with regeneration preservation — v1.6
- Completion flow: done -> enrichment chat (seeded) -> extraction -> profile update -> re-run — v1.6
- Violet-accented action UI on matches page and dashboard — v1.6
- Unified prompt system with markdown body, dynamic fields, three visibility modes — v2.0
- AI learning sidebar with Socratic feedback, module context, and proactive feedback on prompt submit — v2.0
- Facilitator agent with 10 tools and propose-and-approve workflow for comments, messages, pairs — v2.0
- Live session runner with phases, timers, pairing (random/complementary/manual), real-time presence — v2.0
- Module enhancements: essential/optional materials, audio upload with inline playback, time-to-session, continue-here — v2.0
...
**Current state:** v2.0 shipped 2026-03-11. Course program platform complete with unified prompts, AI learning sidebar, facilitator agent, live session runner, and module enhancements. Ready for BAISH Technical AI Safety course delivery.
...
1. Individuals — get matching + recommendations + career actions, keep profiles updated
...
| Decision                                                                    | Rationale                                                                            | Outcome |
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------- |
| Fire-and-forget action generation via scheduler                             | Non-blocking, silent failure — never affects match computation                       | ✓ Good  |
| Growth area aggregation from match recommendations (not persisted)          | Growth areas computed per-batch, not stored per-match — aggregate at generation time | ✓ Good  |
| Shadow-mode Zod for action generation                                       | Consistent with matching pipeline pattern                                            | ✓ Good  |
| verifyActionOwnership DRY helper                                            | 7 public mutations need same auth+ownership check                                    | ✓ Good  |
| Self-contained CareerActionsSection (fetches own data)                      | Simplifies integration into both matches page and dashboard                          | ✓ Good  |
| cancelAction mutation (not in original plan)                                | In-progress -> active transition needs startedAt cleared                             | ✓ Good  |
| buildProfileContext shared helper                                           | DRY profile context between sendMessage and sendCompletionMessage                    | ✓ Good  |
| Completion messages via by_action index (no changes to existing enrichment) | Isolates completion chat without touching general enrichment flow                    | ✓ Good  |
| Reuse EnrichmentChat + ExtractionReview directly                            | Zero component duplication — completion flow uses existing components as-is          | ✓ Good  |
| ActionData interface defined locally                                        | Type safety without tight coupling to Convex generated types                         | ✓ Good  |

---

_Last updated: 2026-03-11 after v2.0 milestone completed_
