...
const profileExtractionTool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      seeking: {
        type: "string",
        description: "What the user is looking for (roles, opportunities, mentorship)"
      }
    },
...
export const sendMessage = action({
  ...
  handler: async (ctx, { profileId, message }) => {
    ...
    const messages = await ctx.runQuery(internal.enrichment.conversation.getMessages, { profileId });

    // Get profile context
    const profile = await ctx.runQuery(internal.profiles.getInternal, { profileId });
...
const COMPLETENESS_SECTIONS = [
  ...
  { id: 'careerGoals', label: 'Career Goals', fields: ['careerGoals', 'aiSafetyInterests', 'seeking'] },
  { id: 'enrichment', label: 'Profile Enrichment', fields: ['hasEnrichmentConversation'] },
