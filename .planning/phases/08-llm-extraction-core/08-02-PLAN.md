...
export const extractFromPdf = action({
  ...
  handler: async (ctx, { documentId }) => {
    ...
    try {
      ...
      const doc = await ctx.runQuery(internal.extraction.queries.getDocument, { documentId });
      ...
      const taxonomy = await ctx.runQuery(internal.extraction.queries.getSkillsTaxonomy);
      ...
      return { success: true, extractedData: { ...extractedData, skills: matchedSkills } };
    } catch (error) {
      // Mark as failed
      await ctx.runMutation(internal.extraction.mutations.updateDocumentStatus, {
        documentId,
