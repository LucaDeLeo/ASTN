...
export const getSuggestedOrgs = query({
  ...
  handler: async (ctx) => {
    ...
    return [...localOrgs, ...globalOrgs].filter(
      (org) => !joinedOrgIds.has(org._id),
    )
  },
})
```
  ...
  return (
    <div className="flex h-[calc(100vh-65px)]">
      ...
      <div className="hidden lg:block w-1/2 h-full">
        <OrgMap onOrgSelect={setSelectedOrg} selectedOrgId={selectedOrg} />
      </div>

      {/* List - right side, scrollable */}
      <div className="flex-1 overflow-y-auto p-4">
        <OrgFilters />
        <OrgList selectedOrgId={selectedOrg} onOrgSelect={setSelectedOrg} />
      </div>
    </div>
  )
}
    ...
    <div className="flex items-center justify-between">
      <div className="space-y-0.5">
        ...
        <p className="text-sm text-slate-500">
          Get org suggestions based on your city. Your exact location is never
          shared.
        </p>
      </div>
      <Switch checked={isDiscoverable} onCheckedChange={handleToggle} />
    </div>
  )
}
```
