...
must_haves:
  truths:
    - "Users with daily digest enabled receive event digest at 9 AM local time"
    - "Users with weekly digest enabled receive event digest on Sunday evening"
    - "Users with 'all' frequency receive in-app notifications when new events are created"
    - "Digest emails group events by organization"
    - "Digest emails show event title, date, org, location, and RSVP link"
  artifacts:
  ...
  key_links:
    ...
    - from: "convex/events.ts"
      ...
      via: "notifyAllFrequencyUsers call after event sync"
...
<task type="auto">
  ...
  <files>convex/notifications/realtime.ts, convex/events.ts</files>
  <action>
...
export const getUsersForAllFrequency = internalQuery({
  ...
  handler: async (ctx, { orgId }) => {
    ...
    for (const membership of memberships) {
      ...
      if (profile.eventNotificationPreferences?.mutedOrgIds?.includes(orgId)) continue;
...
**In convex/events.ts (or wherever lu.ma sync creates events):**

Find the mutation/action that creates events from lu.ma sync (likely in Phase 12). After successfully inserting a new event, call the real-time notification:

```typescript
import { internal } from "./_generated/api";

// After inserting new event in sync logic:
if (isNewEvent) {
  await ctx.scheduler.runAfter(0, internal.notifications.realtime.notifyAllFrequencyUsers, {
    eventId: newEventId,
    orgId: event.orgId,
  });
}
...
Note: Only trigger for genuinely NEW events, not updates to existing events.
  </action>
  <verify>
1. Run `bun run lint` - no TypeScript errors
2. Check that events.ts (or lu.ma sync file) imports and calls notifyAllFrequencyUsers
  </verify>
