...
**IMPORTANT:** This mutation needs to be called from the frontend when users view events. This wiring is done in Task 4.
</action>
...
const ONE*WEEK = 7 * 24 _ 60 _ 60 _ 1000;
const ONE_DAY = 24 _ 60 _ 60 _ 1000;
const ONE*HOUR = 60 * 60 \* 1000;
...
<task type="auto">
  <name>Task 4: Wire recordEventView from event display components</name>
  <files>src/components/events/EventCard.tsx</files>
  <action>
The recordEventView mutation must be called when users view events to enable reminder scheduling. Without this wiring, reminders will never be scheduled because the eventViews table won't have records.

**Find and modify the EventCard component** (or equivalent event display component used on dashboard and org events pages):

1. Import useMutation and the api:
...
2. Add the mutation hook inside the component:

```typescript
const recordView = useMutation(api.notifications.mutations.recordEventView)
```

3. Call recordView when the event card becomes visible OR when user clicks to expand/view details. Choose the appropriate trigger:

**Option A - On card click/expand (recommended):**
If EventCard has an expand or "view details" interaction, call recordView there:

```typescript
const handleViewEvent = () => {
  recordView({ eventId: event._id })
  // ... existing expand/detail logic
}
```

**Option B - On mount with useEffect (for events shown in detail view):**
If this is a detail view component that shows one event:
...
**Option C - Using Intersection Observer (for list views):**
If events are in a scrollable list, use intersection observer to call when card enters viewport:
...
// Then add ref={cardRef} to the card container
```

Choose the option that best fits the existing component pattern. The key requirement: recordEventView MUST be called when user "sees" an event, so reminder scheduling can occur.

**Note:** The mutation is idempotent - it checks for existing views before inserting, so calling it multiple times for the same event is safe.
</action>
<verify>

1. Run `bun run lint` - no TypeScript errors
2. Start dev server, navigate to dashboard or org events page
3. View an event (click/scroll into view depending on implementation)
4. Check Convex dashboard - eventViews table should have a new record
5. If user has reminder preferences enabled, scheduledReminders table should have entries
   </verify>
   <done>recordEventView is called from frontend when users view events, enabling reminder scheduling based on user preferences</done>
   </task>
...
- [ ] EventCard (or equivalent) calls recordEventView when user views event
- [ ] eventViews table populated when viewing events
- [ ] scheduledReminders table populated for users with reminder preferences
...
6. Viewing an event creates eventViews record and schedules reminders
