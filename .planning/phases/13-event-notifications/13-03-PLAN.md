...
must_haves:
  ...
  artifacts:
    ...
    - path: "convex/notifications/scheduler.ts"
      provides: "scheduleEventReminders, cancelEventReminders, sendReminder functions"
      exports: ["scheduleEventReminders", "sendReminder"]
 ...
 * Called after recordEventView
 */
export const scheduleRemindersForView = mutation({
  args: { eventId: v.id("events") },
  handler: async (ctx, { eventId }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) return;
...
- [ ] scheduleRemindersForView creates scheduled functions
