...
7. Action slot for optional CTA button
   ...
   <verify>
8. `bun run lint` - TypeScript compiles
9. Check /opportunities with no data - verify warm styling
10. Test each variant:
    - `<Empty variant="no-data" />`
    - `<Empty variant="no-results" />`
    - `<Empty variant="error" />`
    - `<Empty variant="success" />`
11. Verify backward compat: Existing usage with compound components still works
12. Toggle dark mode - illustrations should adapt (coral remains visible)
    </verify>
