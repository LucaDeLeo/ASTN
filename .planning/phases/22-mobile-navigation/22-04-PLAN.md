...
files_modified:
  ...
  - src/routes/settings/route.tsx # Layout file - wraps Outlet, not page content
autonomous: false
...
**Pattern for PAGE routes (index.tsx, opportunities/index.tsx, matches/index.tsx, profile/index.tsx):**
...
**Pattern for LAYOUT route (settings/route.tsx) - DIFFERENT APPROACH:**
settings/route.tsx is a LAYOUT file that wraps <Outlet />. It currently renders AuthHeader for all child routes. The pattern must be different:

1. Import useIsMobile from ~/hooks/use-media-query
2. Import MobileShell from ~/components/layout/mobile-shell
3. Import useQuery and get user data
4. Conditionally wrap the Authenticated content:
...
function SettingsLayout() {
  ...
  // Build the authenticated content that wraps Outlet
  ...
  // Desktop: keep existing AuthHeader pattern
  return (
    <div className="min-h-screen bg-slate-50">
      <AuthHeader />
      <AuthLoading>...</AuthLoading>
      <Unauthenticated>...</Unauthenticated>
      <Authenticated>{authenticatedContent}</Authenticated>
    </div>
  )
}
```

Key difference: For layout routes, MobileShell wraps <Outlet />, not page content. Do NOT modify settings/index.tsx.
...
1. Check each PAGE file (index.tsx, opportunities/index.tsx, matches/index.tsx, profile/index.tsx) for useIsMobile conditional rendering
2. Check settings/route.tsx for layout-specific pattern (MobileShell wrapping Outlet inside Authenticated)
3. Confirm MobileShell is used for authenticated mobile views
4. Confirm AuthHeader remains for desktop views
5. Run `bun run lint` to check for type errors
   </verify>
   <done>5 primary routes conditionally render MobileShell on mobile and AuthHeader on desktop (4 page routes + 1 layout route)</done>
   </task>
