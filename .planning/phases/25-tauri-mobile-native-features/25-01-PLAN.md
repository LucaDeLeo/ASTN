...
```
  ...
  <verify>
1. Check that src-tauri/ directory exists: `ls src-tauri/`
2. Verify plugin initialization in lib.rs: `grep "mobile_entry_point" src-tauri/src/lib.rs`
3. Verify deep-link capability: `grep "deep-link:default" src-tauri/capabilities/default.json`
4. Verify app identifier: `grep "ai.astn.app" src-tauri/tauri.conf.json`
5. Verify all plugins in lib.rs: `grep -E "plugin_deep_link|plugin_store|plugin_os|plugin_notification" src-tauri/src/lib.rs`
  </verify>
