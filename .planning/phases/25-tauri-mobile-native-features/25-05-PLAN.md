...
async function getStore(): Promise<StoreInterface> {
  if (isTauri()) {
    ...
    return {
      ...
      set: async <T>(key: string, value: T) => { await store.set(key, value) },
...
export async function checkBiometricAvailability(): Promise<BiometricStatus> {
  ...
  try {
    ...
    if ('biometryType' in status) {
      ...
      else if (type === 'touchId' || type === 'TouchID') biometryType = 'touchId'
...
Now integrate into src/routes/__root.tsx. Add the biometric unlock layer at the root:
...
Note: The exact integration depends on the current __root.tsx structure. Wrap the main content, not the entire router.
  </action>
  <verify>
