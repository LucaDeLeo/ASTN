  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(OPPORTUNITIES_STORE)) {
        const store = db.createObjectStore(OPPORTUNITIES_STORE, {
          keyPath: 'id',
        })
 ...
 */
export async function cacheOpportunity(id: string, data: any): Promise<void> {
  try {
 ...
 */
export async function getCachedOpportunity(id: string): Promise<any | null> {
  try {
    const db = await openDatabase()
...
export const sendPushNotification = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!fcmServerKey) {
      console.warn(
        'FIREBASE_SERVER_KEY not configured, skipping push notification',
      )
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12">
      <p className="text-muted-foreground">
        This opportunity hasn't been cached for offline viewing.
      </p>
