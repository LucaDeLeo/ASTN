...
must_haves:
  truths:
    ...
    - "User does not see 'Not Found' for salary (graceful omission or fallback)"
...
- Change: `{match.opportunity.organization} · {match.opportunity.location}`
- To: `{match.opportunity.organization} · {formatLocation(match.opportunity.location)}`
...
- To: `{formatLocation(opportunity.location)}`
  ...
  <verify>
1. Verify file created: `test -f src/lib/formatLocation.ts && echo "File exists"`
2. Run `bun run lint` to verify TypeScript compiles
3. Run the dev server: `bun run dev`
4. Navigate to /opportunities and verify location strings show comma separators
5. Navigate to /matches and verify location strings are consistent
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: Replace "Not Found" salary with graceful display</name>
  <files>src/components/matches/MatchCard.tsx, src/components/opportunities/opportunity-card.tsx</files>
  <action>
Replace "Not Found" salary displays with graceful handling - either omit the field entirely or show "Salary not specified".

1. In `src/components/matches/MatchCard.tsx`:
- Find where salary is displayed (likely in the card details)
- If salary value is "Not Found", null, undefined, or empty string, don't render the salary line at all
- Pattern:
```tsx
{opportunity.salary && opportunity.salary !== "Not Found" && (
  <span>{opportunity.salary}</span>
)}
```

2. In `src/components/opportunities/opportunity-card.tsx`:
- Apply same pattern - only show salary if it has a meaningful value
- Avoid showing "Not Found" to users

3. Alternative approach if salary should always show:
- Replace "Not Found" with "Salary not specified" or "Contact for salary"
```tsx
{opportunity.salary && opportunity.salary !== "Not Found"
  ? opportunity.salary
...
Choose the approach that matches existing UX patterns in the codebase (check how other optional fields are handled).
  </action>
  <verify>
1. Run the dev server: `bun run dev`
2. Navigate to /opportunities and find cards that had "Not Found" salary
3. Verify they now either omit salary or show graceful message
4. Navigate to /matches and verify same behavior
5. Run `bun run lint` - passes
  </verify>
  <done>
- No cards display "Not Found" for salary
- Salary either omitted when unknown or shows graceful fallback message
  </done>
</task>

<task type="auto">
  <name>Task 3: Make MatchCard fully clickable</name>
...
<task type="auto">
  <name>Task 4: Add navigation active state indicators</name>
...
2. "Not Found" salary displays replaced with graceful handling
3. Entire match card is clickable, not just the title
4. Desktop navigation shows which page is active via underline indicator
5. No regression in existing functionality (view transitions, save/unsave)
</success_criteria>
