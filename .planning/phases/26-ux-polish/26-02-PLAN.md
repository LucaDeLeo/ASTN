...
2. Verify bun.lock updated: `grep -q "space-grotesk" bun.lock && echo "Lock file updated"`
3. Run the dev server: `bun run dev`
4. Check that headings appear in Space Grotesk (geometric, technical look vs Lora's serif)
5. Check the network tab - Space Grotesk woff2 should load
6. Run `bun run lint` - no TypeScript errors
  </verify>
  <done>
