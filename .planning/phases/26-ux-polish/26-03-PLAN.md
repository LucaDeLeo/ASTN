...
1. Verify each variant shows unique illustration
1. Verify each variant shows appropriate title and description
...
First, understand how profile completeness is accessed in this codebase:

- The matches page likely already queries profile data via Convex
- Look for existing `useQuery` calls like `useQuery(api.profiles.get)` or similar
- Profile completeness is tracked via `profile.completeness.overallScore` or `profile.completeness.sections`
- Check the existing code for how profile data is accessed

Determine `hasCompleteProfile`:

```tsx
// Find the existing profile query in the component
const profile = useQuery(api.profiles.get) // or similar existing query

// Profile completeness check (adjust based on actual completeness structure)
const hasCompleteProfile = profile?.completeness?.overallScore >= 0.7 // or check specific sections
```
...
Determine filter state from existing component code:

- Look for existing state like `const [searchQuery, setSearchQuery] = useState("")`
- Look for filter state like `const [filters, setFilters] = useState({...})`
- Or URL search params via TanStack Router: `const { search } = Route.useSearch()`

Example pattern (adapt to actual code structure):

```tsx
// If using URL search params
const { q: searchQuery, ...filters } = Route.useSearch()
const hasFilters = Object.values(filters).some(
  (v) => v !== undefined && v !== '',
)

// If using local state, check existing state variables
// const hasFilters = selectedCategory || selectedLocation || searchQuery;

// Find the existing clearFilters function or create one:
