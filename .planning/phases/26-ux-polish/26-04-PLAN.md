...
--navy-900: oklch(0.2 0.04 250); /* Deepest navy - text on light bg */
--navy-800: oklch(0.25 0.04 250); /* Primary buttons */
--navy-700: oklch(0.32 0.04 250); /* Hover states */
--navy-600: oklch(0.4 0.03 250); /* Secondary elements */
--slate-500: oklch(0.5 0.02 250); /* Muted text */
--slate-400: oklch(0.6 0.02 250); /* Borders, dividers */
--slate-300: oklch(0.7 0.01 250); /* Light backgrounds */
...
--accent: oklch(0.7 0.16 30); /* Original coral */
...
.gradient-bg-radial {
  background: radial-gradient(
    ...
    oklch(0.96 0.01 250) 70%,
    oklch(0.94 0.015 250) 100%
  );
  background-image:
    radial-gradient(
      ellipse at center,
...
.dark {
  ...
  --gradient-linear: linear-gradient(
    135deg,
    oklch(0.2 0.01 250) 0%,
    oklch(0.16 0.005 250) 100%
  );
  --gradient-subtle: linear-gradient(
    180deg,
    oklch(0.19 0.008 250) 0%,
    oklch(0.16 0.005 250) 100%
  );
}
```
...
```css
:root {
  --shadow-warm-sm: 0 1px 2px oklch(0.3 0.02 250 / 0.06);
  --shadow-warm: 0 4px 12px oklch(0.3 0.02 250 / 0.08);
  --shadow-warm-md: 0 8px 24px oklch(0.3 0.02 250 / 0.12);
  --shadow-warm-lg: 0 12px 40px oklch(0.3 0.02 250 / 0.15);
}
```
...
5. Scope limitation: Only fix the two files specified above (MatchCard.tsx and opportunity-detail.tsx).
   Do NOT search for and fix other components in this task - that would expand scope unpredictably.
   Any other hardcoded colors can be addressed in a follow-up phase if needed.
   </action>
   <verify>
1. Run the dev server
1. Enable dark mode
1. Navigate through all main pages:
   ...
   - Match detail
   - Opportunity detail
1. Verify all text is readable (no low-contrast text)
1. Check cards, badges, buttons for proper dark mode appearance
