...
5. Create `convex/extraction/validation.ts` -- for document extraction (text.ts, pdf.ts):
   ...
   export type DocumentExtractionResultValidated = z.infer<typeof documentExtractionResultSchema>;
   ```
  </action>
  <verify>
1. `bun add zod@^3.25` completes (check package.json lists zod)
2. `npx convex dev --once` confirms all new .ts files in convex/ compile
3. Each validation file exports its schema and type
4. Schemas use .passthrough() and .optional() generously (permissive mode)
  </verify>
  <done>
...
4. `convex/engagement/prompts.ts` -- Modify `buildEngagementContext()`:
   - After `sections.push(\`## Engagement Classification for ${orgName}\n\`);` add:
     `sections.splice(0, 0, "<member_data>");` (insert at beginning)
...
2. `convex/enrichment/conversation.ts` -- Add profile field length validation before LLM call:
   ...
   - Alternatively, if the existing code cannot easily be changed to `let`, create a new variable:
     ```typescript
     const truncatedContext = profileContext.length > 50000
...
3. `convex/extraction/text.ts` -- Add document text length limit:
  ...
  <verify>
1. `npx convex dev --once` compiles all modified files
2. `bun run build` succeeds
3. XML delimiter checks:
...
5. Shadow mode check: `grep "LLM_VALIDATION_FAIL" convex/matching/compute.ts convex/engagement/compute.ts convex/extraction/text.ts convex/extraction/pdf.ts convex/enrichment/extraction.ts` returns 5 matches
6. Input limit check: `grep "FIELD_LIMITS" convex/enrichment/conversation.ts convex/extraction/text.ts` returns matches
  </verify>
  <done>
