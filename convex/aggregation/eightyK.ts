...
export const fetchOpportunities = internalAction({
  ...
  handler: async (): Promise<Array<NormalizedOpportunity>> => {
    if (!ALGOLIA_APP_ID || !ALGOLIA_API_KEY) {
      log('error', 'Missing 80K Hours Algolia credentials')
      return []
    }
    ...
    try {
      ...
      log('info', 'Fetched opportunities from 80K Hours', {
        count: results.length,
      })
      return results.map(normalizeEightyKJob)
    } catch (error) {
      log('error', 'Error fetching from 80K Hours', { error: String(error) })
      return []
    }
  },
})
