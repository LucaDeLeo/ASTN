...
type EngagementWithOverride = {
  level: string
  override?: { level: string; expiresAt?: number; notes?: string }
}

// Check whether an override exists and has not expired.
function isOverrideActive(engagement: EngagementWithOverride): boolean {
  if (!engagement.override) return false
  if (
    engagement.override.expiresAt &&
    engagement.override.expiresAt < Date.now()
  )
    return false
  return true
}
...
function getEffectiveLevel(engagement: EngagementWithOverride): string {
  if (isOverrideActive(engagement)) {
...
export const getMemberEngagement = query({
  ...
  handler: async (ctx, { orgId }) => {
    ...
    return {
      ...
      hasOverride: isOverrideActive(engagement),
    }
  },
})
...
export const getOrgEngagementForAdmin = query({
  ...
  handler: async (ctx, { orgId }) => {
    ...
    return engagementRecords.map((e) => ({
      ...
      hasOverride: isOverrideActive(e),
