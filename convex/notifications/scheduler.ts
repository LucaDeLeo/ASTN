...
export const scheduleRemindersForViewInternal = internalMutation({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    const scheduleReminder = async (timing: "1_week" | "1_day" | "1_hour", offset: number) => {
...
export const cancelEventReminders = internalMutation({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    for (const reminder of reminders) {
      try {
        await ctx.scheduler.cancel(reminder.scheduledFunctionId as Id<"_scheduled_functions">);
      } catch {
