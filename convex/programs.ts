...
export const updateProgram = mutation({
  args: {
    ...
    enrollmentMethod: v.optional(
      v.union(v.literal("admin_only"), v.literal("self_enroll"), v.literal("approval_required")),
    ),
    maxParticipants: v.optional(v.number()),
    completionCriteria: v.optional(
      v.object({
...
export const enrollMember = mutation({
  ...
  handler: async (ctx, { programId, userId, adminNotes }) => {
    ...
    const userMembership = await ctx.db
      .query("orgMemberships")
      .withIndex("by_user_and_org", (q) => q.eq("userId", userId).eq("orgId", program.orgId))
      .first();
    ...
    const existing = await ctx.db
      .query("programParticipation")
      .withIndex("by_program_and_user", (q) => q.eq("programId", programId).eq("userId", userId))
      .first();
  ...
  handler: async (ctx, { programId }) => {
    ...
    const acceptedApps = await ctx.db
      ...
      .withIndex("by_opportunity_and_status", (q) =>
        q.eq("opportunityId", program.linkedOpportunityId!).eq("status", "accepted"),
      )
      .collect();
...
export const getLinkedOpportunityInfo = query({
  ...
  handler: async (ctx, { programId }) => {
    ...
    const opp = await ctx.db.get("orgOpportunities", program.linkedOpportunityId);
    if (!opp) return null;
...
export const toggleMaterialProgress = mutation({
  ...
  handler: async (ctx, { moduleId, materialIndex }) => {
    ...
    const existing = await ctx.db
      ...
      .withIndex("by_module_and_user", (q) => q.eq("moduleId", moduleId).eq("userId", userId))
      .collect();
...
export const getProgramModules = query({
  ...
  returns: v.array(
    v.object({
      ...
      status: v.union(v.literal("locked"), v.literal("available"), v.literal("completed")),
      createdAt: v.number(),
      updatedAt: v.number(),
    }),
  ),
...
export const createModule = mutation({
  args: {
    ...
    linkedSessionId: v.optional(v.id("programSessions")),
    status: v.optional(
      v.union(v.literal("locked"), v.literal("available"), v.literal("completed")),
    ),
  },
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .collect();
    const maxOrder = existing.reduce((max, m) => Math.max(max, m.orderIndex), -1);
...
export const updateModule = mutation({
  ...
  handler: async (ctx, { moduleId, ...updates }) => {
    ...
    if (updates.materials !== undefined) {
      const oldStorageIds = (mod.materials ?? []).map((m) => m.storageId).filter(Boolean);
...
export const deleteModule = mutation({
  ...
  handler: async (ctx, { moduleId }) => {
    ...
    for (const mat of mod.materials ?? []) {
      if (mat.storageId) {
        await ctx.storage.delete(mat.storageId as Id<"_storage">);
      }
    }
...
async function checkProgramAccess(
  ...
  const participation = await ctx.db
    .query("programParticipation")
    .withIndex("by_program_and_user", (q) => q.eq("programId", program._id).eq("userId", userId))
    .first();

  if (
    participation &&
    (participation.status === "enrolled" || participation.status === "completed")
  ) {
...
async function requireProgramAccess(ctx: QueryCtx, program: Doc<"programs">): Promise<string> {
...
export const getProgramBySlug = query({
  ...
  handler: async (ctx, { orgId, programSlug }) => {
    const program = await ctx.db
      .query("programs")
      .withIndex("by_org_slug", (q) => q.eq("orgId", orgId).eq("slug", programSlug))
      .first();
    ...
    // Get linked events
    const validEvents = await fetchLinkedEvents(ctx, program.linkedEventIds ?? []);
...
async function fetchLinkedEvents(ctx: QueryCtx, linkedEventIds: Array<Id<"events">>) {
  const events = await Promise.all(linkedEventIds.map((id) => ctx.db.get("events", id)));
  return events
...
export const getProgram = query({
  ...
  handler: async (ctx, { programId }) => {
    ...
    const participants = await ctx.db
      .query("programParticipation")
      .withIndex("by_program_status", (q) => q.eq("programId", programId).eq("status", "enrolled"))
      .collect();
...
export const getOrgEventsForLinking = query({
  ...
  handler: async (ctx, { orgId }) => {
    ...
    const events = await ctx.db
      .query("events")
      .withIndex("by_org_start", (q) => q.eq("orgId", orgId).gte("startAt", threeMonthsAgo))
      .collect();
