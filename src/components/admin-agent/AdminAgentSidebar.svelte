<script lang="ts">
  ...
  import AdminAgentChat from './AdminAgentChat.svelte'
...
{#if $isMobile}
  {#if agent.isOpen}
    <div
      ...
      role="button"
      tabindex="0"
      aria-label="Close admin agent overlay"
    ></div>
    <div class="fixed inset-x-0 bottom-0 z-50 h-[84dvh] rounded-t-[2rem] border border-border/70 bg-background shadow-2xl">
      <div class="flex items-center justify-between border-b border-border/70 px-4 py-3">
        <div>
          ...
          <p class="text-sm text-slate-600">Admin operations copilot</p>
        </div>
        <button
          type="button"
...
{:else}
  <button
    ...
    aria-label={agent.isOpen ? 'Close admin agent' : 'Open admin agent'}
  >
    {#if agent.isOpen}
