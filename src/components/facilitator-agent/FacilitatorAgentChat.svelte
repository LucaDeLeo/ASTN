<script lang="ts">
  ...
  import { Copy, Send, Terminal, Trash2 } from 'lucide-svelte'
  ...
  const command = $derived(`bun agent/cli.ts --org=${agent.orgSlug} --program=${agent.programId}`)
</script>
...
{#if agent.status === 'disconnected'}
  <div class="flex h-full flex-col items-center justify-center px-6 text-center">
    ...
    <p class="mt-2 max-w-sm text-sm text-slate-600">
      Start the facilitator agent from your terminal to connect this sidebar.
    </p>
      ...
      {#if !agent.messages.length && !agent.isStreaming}
        <div class="flex h-full items-center justify-center text-center">
          <p class="max-w-sm text-sm leading-6 text-slate-500">
            Ask about participant progress, facilitation plans, attendance, or program health.
          </p>
        </div>
      {/if}
