...
function DisconnectedView({
  ...
  const command = `bun agent/cli.ts --org=${orgSlug} --program=${programId}`
      ...
      <div className="max-w-sm w-full space-y-6 text-center">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Start the facilitator agent from your terminal to connect.
          </p>
        </div>
...
export function FacilitatorAgentChat({
  agent,
  orgSlug,
  programId,
}: {
  ...
  const [model, setModel] = useState<AgentModel>('claude-sonnet-4-6')
  ...
  if (status === 'disconnected') {
    return <DisconnectedView orgSlug={orgSlug} programId={programId} />
  }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 min-h-0">
        {messages.length === 0 && !isStreaming && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <p className="text-sm text-muted-foreground">
              Ask the facilitator agent about participant progress, responses,
              or session prep.
            </p>
          </div>
        )}
      ...
      <div className="border-t px-4 py-3 shrink-0">
        <div className="flex items-end gap-2">
          <textarea
            ...
            placeholder="Ask the facilitator agent..."
            disabled={isStreaming}
            rows={1}
            className={cn(
