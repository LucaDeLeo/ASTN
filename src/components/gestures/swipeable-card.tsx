...
export function SwipeableCard({
  ...
  // Indicator opacities
  const leftOpacity =
    phase === 'exiting' && swipeDirection === 'left'
      ? 1
      : Math.min(Math.abs(Math.min(offset, 0)) / SWIPE_THRESHOLD, 1)
  const rightOpacity =
    phase === 'exiting' && swipeDirection === 'right'
      ? 1
      : Math.min(Math.max(offset, 0) / SWIPE_THRESHOLD, 1)
  ...
  const getCardTransform = () => {
    ...
    if (
      ['undo-settling', 'undo-visible', 'undo-fading', 'collapsing'].includes(
        phase,
      )
    ) {
      return -500
  ...
  const getContainerHeight = () => {
    ...
    if (['undo-settling', 'undo-visible', 'undo-fading'].includes(phase))
      return 56
  ...
  const showTrailGhost =
    (phase === 'swiping' || phase === 'exiting') && Math.abs(offset) > 30
  const showMainCard = [
    'idle',
    'swiping',
    'exiting',
        ...
        >
          <div
            ...
            style={{
              ...
              transition:
                phase === 'exiting' ? 'transform 300ms ease-out' : 'none',
            }}
          >
            {offset < 0 ? (
