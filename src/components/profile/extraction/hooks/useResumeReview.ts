 ...
 */
function formatEducationLabel(
  edu: NonNullable<ExtractedData['education']>[0],
): string {
...
function formatWorkLabel(
  work: NonNullable<ExtractedData['workHistory']>[0],
): string {
...
export function useResumeReview(
  ...
  const updateValue = useCallback((id: string, value: unknown) => {
    setItems((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, editedValue: value, status: 'edited' }
          : item,
      ),
    )
  }, [])
